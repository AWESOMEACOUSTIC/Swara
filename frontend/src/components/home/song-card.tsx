...
export function SongCard({ song }: { song: SongWithRelation }) {
  ...
  const handlePlay = async () => {
    ...
    setTrack({
      ...
      title: song.title,
      url: playUrl,
      artwork: song.thumbnailUrl ?? null,
      prompt: song.prompt,
  ...
  return (
    <div>
      <div onClick={handlePlay} className="cursor-pointer">
        ...
        <h3 className="mt-2 truncate text-sm font-medium text-stone-100">
          {song.title}
        </h3>

        <p className="text-xs text-zinc-400">{song.user.name}</p>

        <div className="mt-1 flex items-center justify-between text-xs text-violet-400">
          ...
          >
            <Heart
              className={`h-4 w-4 ${isLiked ? "text-fuchsia-400" : " hover:text-fuchsia-300 "}`}
            />
            {likesCount} likes
          </button>
        </div>
